...
/** Read manifest from Blob only (no flyer sync). */
...
/** Load manifest and merge with all images currently in Blob. */
export async function loadManifest(options = {}) {
  const { reconcile = true, save = true } = options;

  let manifest = await loadManifestFromStore();

  if (!reconcile) return manifest;

  const { reconcileManifestWithBlob } = await import("./sync-blob.mjs");
  const result = await reconcileManifestWithBlob(manifest, { save });
...
export async function resolvePathnameUrl(pathname) {
...
async function resolveItemUrl(item) {
  ...
  try {
    return await resolvePathnameUrl(pathname);
  } catch {
    return item.url;
  }
}
...
async function resolveItemUrls(item) {
  const fullPromise = resolveItemUrl(item);
  const thumbPath = item.thumbPathname;
  const thumbPromise = thumbPath
    ? resolvePathnameUrl(thumbPath).catch(() => null)
    : null;

  const url = await fullPromise;
  const thumbUrl = thumbPromise ? await thumbPromise : null;
...
export async function buildPublicPayload(manifest) {
  ...
  await Promise.all(
    sorted.map(async (item) => {
      Object.assign(item, await resolveItemUrls(item));
    })
  );
